...
def area(label_image: np.ndarray) -> int:
    return np.sum(label_image > 0)


def pixels_list(label_image: np.ndarray) -> np.ndarray:
    return np.stack(label_image.nonzero(), axis=-1)


def centroid(label_image: np.ndarray) -> np.ndarray:
    return pixels_list(label_image).mean(axis=0)
